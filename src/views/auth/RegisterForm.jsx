...
const RegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure resume file is a PDF
    if (resume && resume.type !== 'application/pdf') {
      seterrMsg('Resume should be a PDF file.');
      return;
    }
    ...
    try {
      ...
      await axios.post('https://api.ctythat.com/api/User', formData, {
      ...
      if (error.response) {
        ...
        } else if (errorMessage.toLowerCase().includes('invalid file type')) {
          seterrMsg('Resume should be a PDF file.');
        } else {
          seterrMsg(errorMessage || 'Email already exists. Please use a different email.');
        }
      } else {
      ...
      >
        {loading ? (
          <div className="flex flex-col justify-center items-center">
            ...
            <p className="mt-4 text-gray-700 font-semibold">Registering... Please wait</p>
          </div>
        ) : (
          <>
            ...
            <p ref={errRef} className={errMsg ? 'my-6 text-red-800 font-semibold bg-red-300 text-sm text-center py-1 border-1 border border-red-800' : 'hidden'} aria-live="assertive">
              {errMsg}
            </p>
